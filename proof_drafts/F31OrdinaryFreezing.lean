...
Besides the two explicit external Weyl premises, the final theorem still requires
the unproved INTERNAL core statement MonomialDenseReturns. This is not a proof of P0. -/
...
/-- Finite ordinary degree induction isolates the one remaining nonlinear core obligation.
This theorem is conditional on hR; it does not establish that obligation or general freezing. -/
