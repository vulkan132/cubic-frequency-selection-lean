...
/-! Actual native lower-central rationality and bounded layer bases
assembled with the original weak basis, lattice and smooth exponential. -/
...
/-- One selection from the original structural presentation supports the
weak basis, rational exponential arrays and all actual native lower-central
layers. Bounds for a finite number of layers precede the layer index. -/
theorem original_full_bounded_lie_filtration
  ...
  obtain ⟨d, bZ, bR, logCoord, S, T, Q, hb, hweak, hS, hT, hzero, hbounds, hLieData⟩ :=
    original_full_bounded_weak_basis V Gamma coord b hint hcover p hjoint q htri hlow P hP
  ...
  refine ⟨hLieData.1, hLieData.2.1, hLieData.2.2, ?_, ?_⟩
  · exact original_weak_basis_lower_central_rational (observationFullCoordinates V coord bR)
      (observationLatticeSubgroup V Gamma) logCoord Q hweak hLieData.1
  · exact original_weak_basis_lower_central_bases (observationFullCoordinates V coord bR)
      (observationLatticeSubgroup V Gamma) logCoord Q hweak hLieData.1
