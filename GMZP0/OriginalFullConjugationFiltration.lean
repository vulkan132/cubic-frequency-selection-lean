...
/-! Conjugation stability of actual exponential lower-central sets,
retaining all original F64 witnesses and lattice-span conclusions. -/
...
/-- The same original structural construction now also proves conjugation
invariance of every exponential lower-central set. All F64 witnesses and
finite-depth bounds are retained; multiplication closure is still separate. -/
theorem original_full_conjugation_lie_filtration
      ...
      ∃ S T : (Fin m ⊕ Fin d) → MvPolynomial (Fin m ⊕ Fin d) ℚ, ∃ Q : ℕ,
        ...
        (letI := (observationFullCoordinates V coord bR).isOpenEmbedding.singletonChartedSpace;
          ...
          (∀ n : ℕ, let U := (LieModule.lowerCentralSeries ℝ
            ...
            OriginalExponentialSubspaceData (observationLatticeSubgroup V Gamma) logCoord U ∧
              ∀ g h : ObservationGroup V,
                (g * h * g⁻¹ ∈ originalExponentialImage logCoord U ↔
                  h ∈ originalExponentialImage logCoord U)) ∧
  ...
  exact ⟨hLie, hExp, hLog,
    original_lower_central_conjugation_data (observationFullCoordinates V coord bR)
