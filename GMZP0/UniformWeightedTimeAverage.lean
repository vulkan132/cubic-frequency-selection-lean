...
/-! A uniform positive weighted average of complete cubic t sums, with all original responses. -/

noncomputable section
open scoped BigOperators
namespace GMZP0

theorem uniform_positive_weighted_time_average (κ η : ℝ) (hκ : 0 < κ) (hη : 0 < η) (hη1 : η ≤ 1) :
    ∃ M : ℕ, ∃ c a : ℝ, ∃ N₀ : ℕ,
      0 < M ∧ 0 < c ∧ 0 < a ∧ 0 < N₀ ∧
      ∀ N : ℕ, N₀ ≤ N → ∀ (q : ℕ) [NeZero q], N ^ 2 < q → q ≤ 128 * N ^ 2 →
        ...
        ∃ X : Finset (Fin N), ∃ D : Finset (Base N), ∃ b : ℕ,
          ...
          (a / 4) ^ 16 ≤ cyclicWeightedTimeNorm N q (smoothingRadius N a)
            θ (originalScaledWeight N μ D) ∧
          (∀ z ∈ D, μ z ≠ 0 → η ≤ (lam z * response N f z (θ z)).re) := by
  obtain ⟨M, c, a, N₀, hM, hc, ha, hN₀, hwindow⟩ := uniform_positive_cube_average κ η hκ hη hη1
  refine ⟨M, c, a, N₀, hM, hc, ha, hN₀, ?_⟩
  intro N hNth q _ hq hqUpper f θ lam μ hdata
  have hN : 0 < N := hN₀.trans_le hNth
  obtain ⟨X, D, b, hDX, hinterval, hmass, hgap, hsafe, hR, hcube, hresponse⟩ :=
    hwindow N hNth q hq hqUpper f θ lam μ hdata
  refine ⟨X, D, b, hDX, hinterval, hmass, hgap, hsafe, hR, ?_, hresponse⟩
  exact hcube.trans (cyclicCubeAverage_le_weightedTimeNorm N q (smoothingRadius N a)
    θ (originalScaledWeight N μ D) lam (originalScaledWeight_bounds hN μ D hdata.2.2.1) hdata.2.1)
