...
F65 共 1694 条定理已通过完整编译与公理审计，记录见 verification/result.json。
其中包括明确带前提的条件性归约。

F65 新增 18 条定理。实际原群共轭的坐标微分已与原生切空间微分识别，
并证明共轭后的原单参数子群具有该微分作用后的切向量。原唯一性给出
Exp(A_g v)=g Exp(v) g^(-1) 及对应的原对数恒等式，原乘法次序保持。
实际逆微分、左不变向量场拉回和 Mathlib 括号自然性进一步证明 A_g
保持实际原生 Lie 括号，构造原 Lie 同态。每层实际 Lie 下中心列因此
在 A_g 下稳定，其原指数像在全部原群共轭下成员资格双向不变。完整
结构定理保留 F64 的全部原基、原完整格子、拓扑、对数张成、数组界和
有限层高度界。尚未证明这些指数像乘法封闭，故不能称为已构造的正规
子群；原群下中心列对应、格子交余紧性、共同适配基及外部匹配、一般
下降终止、原全标签频率实现和 WeightedCapture/P0 仍 OPEN。详见
ORIGINAL_CONJUGATION_FILTRATION_INTERFACE.md。以下各段保留历史作用域。

F64 新增 15 条定理。实际原对数的同胚限制证明每个线性子空间的指数像
闭且可缩；原单参数子群给出实际逆元与幂公式，并证明全部正整数幂的
指数像成员资格双向等价。统一清除固定有限有理生成族的分母后，得到
实际原格子中的有限点集，其对数位于原有理子空间内并精确张成该空间。
这些结论已接到每层实际原生 Lie 下中心列，并在一次 F63 构造中保留
全部原基、原完整格子、弱基界、双向光滑映射、数组及有限层高度界。
Heisenberg 公式的精确例子得到中心对数坐标 1/2，排除从任意线性空间
直接推断指数像乘法封闭。尚未证明 Lie 理想的指数像为所需子群、原群
下中心列对应或格子交的余紧性；共同适配基、外部匹配、一般下降终止、
原全标签频率实现及 WeightedCapture/P0 仍 OPEN。详见
ORIGINAL_EXPONENTIAL_SUBSPACE_INTERFACE.md。以下各段保留历史作用域。
